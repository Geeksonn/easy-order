...
const createItem = async (item) => {
    ...
    // Step 2: Upload imageCard
    item.imageCard = (await uploadImage(item.imageCard)).uuid;
...
const deleteItem = async (item) => {
    ...
    const imgDelResult = await deleteFile({ uuid: item.image }, { authSchema: client });
    // TODO Error management

    // Step 2: Delete imageCard
    const imgCardDelResult = await deleteFile({ uuid: item.imageCard }, { authSchema: client });
    // TODO Error management
